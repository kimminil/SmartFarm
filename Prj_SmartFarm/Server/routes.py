...
def data_sensor_load():
    ...
    if latest:
        return jsonify({
            'No': latest.No,
            'log_time': latest.log_time.strftime('%Y-%m-%d %H:%M:%S') if latest.log_time else None,
            'temp': latest.temp,
            'humi': latest.humi,
            'co2': latest.co2,
            'light': latest.light,
            'w_height': latest.w_height,
            'cmd_Peltier': latest.cmd_Peltier,
            'cmd_fan': latest.cmd_fan,
            'cmd_light': latest.cmd_light,
            'cmd_w_pump': latest.cmd_w_pump
        })
    else:
