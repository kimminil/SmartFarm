	  ...
	  if(huart->Instance == USART1)
	   {
		 ...
		 __HAL_UART_CLEAR_PEFLAG(&huart1);
		  ...
		  #if DMA_Nomal_EN_Uart_1
		      HAL_UART_Receive_DMA(&huart1, (uint8_t *)rx_buf1, 5);
		  #elif IRQ_EN_Uart_1
		      HAL_UART_Receive_IT(&huart1, (uint8_t *)rx_buf1, 5);
		  #endif
	   }
#endif
  ...
  if(huart->Instance == USART2)
   {
	 ...
	 //---------------------------------------------------------
	  ...
	  #if DMA_Nomal_EN_Uart_2
	      // Nomal Mode시에만 적용
	      HAL_UART_Receive_DMA(&huart2, (uint8_t *)rx_buf2, 1); //rx_d -> buff_size);
	  #elif IRQ_EN_Uart_2
	      HAL_UART_Receive_IT(&huart2, (uint8_t *)rx_buf2, 5); //rx_d -> buff_size);
	  #endif
   }
#endif
  ...
  if(huart->Instance == USART3)
   {
    ...
    tx_send(rx_d -> Rx_data_3[0], 2);

	if(rx_d -> Rx_data_3[0] == '\r')
	 {
	   rx_flag -> rx_end_flag_3 = 1;
	   rx_cnt -> rx_cnt_2 = 0;
	 }
	...
	{
	  // rx data save
	  rx_d -> Rx_data_3[++rx_cnt -> rx_cnt_2] = rx_d -> Rx_data_3[0];
	  rx_d -> Rx_data_3[0] = 0;
	}
  	...
  	#if DMA_Nomal_EN_Uart_3
  	    HAL_UART_Receive_DMA(&huart3, (uint8_t *)rx_d -> Rx_data_3, 5);
  	#elif IRQ_EN_Uart_3
  	    HAL_UART_Receive_IT(&huart3, (uint8_t *)rx_d -> Rx_data_3, 1);
  	#endif
  }
#endif
   ...
   if(huart->Instance == UART4)
    {
   	...
   	#if DMA_EN_Uart_4
   	    HAL_UART_Receive_DMA(&huart4, (uint8_t *)rx_d -> Rx_data_4, 1);
   	#elif IRQ_EN_Uart_4
   ...
   if(huart->Instance == USART5)
    {
      ...
      #if DMA_EN_Uart_5
    	  HAL_UART_Receive_DMA(&huart5, (uint8_t *)rx_d -> Rx_data_5, 20);
      #elif IRQ_EN_Uart_5
    	  HAL_UART_Receive_IT(&huart5, (uint8_t *)rx_d -> Rx_data_5, 10);
      #endif
    }
#endif
...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t Size)
{
 if(huart->Instance == USART1)
 {
  ...
  // rxd_flag = 1;
   printf("rx byte = %d\r\n", Size);
  // HAL_UARTEx_ReceiveToIdle_IT(&huart1, (uint8_t *)rx_d -> Rx_data_1, 100);
  // HAL_UARTEx_ReceiveToIdle_IT(&huart1, (uint8_t *)rx_buf, 100);

   //HAL_UARTEx_ReceiveToIdle_DMA(&huart1, (uint8_t *)rx_d -> Rx_data_1, 100);
   //echo out == uart2
